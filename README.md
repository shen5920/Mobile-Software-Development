...
覆盖**微信小程序**与**鸿蒙应用（HarmonyOS）**两个移动开发平台：前四个实验基于微信小程序原生开发（WXML / WXSS / JavaScript），覆盖页面结构、常用组件、API 调用、数据与状态管理、canvas 绘图、触摸交互、本地缓存等核心知识；实验5 基于 DevEco Studio 进行鸿蒙应用开发（ArkTS / ArkUI 声明式开发），实现功能完整的计算器应用；实验6 回归微信小程序，使用**微信云开发**（云函数 / 云数据库）实现照片分享社区。开发过程中结合 Python / Node.js 工具进行数据验证、素材生成与算法单元测试。
...
| 实&nbsp;验&nbsp;6 | [lab6](lab6/) | 光影集（云开发） | 微信云开发照片分享社区：上传照片、点赞、评论、个人主页，头像昵称登录 |
...
### 实验6：光影集 · 微信云开发照片社区（lab6）

基于微信小程序云开发（云函数 / 云数据库 / 云存储）实现的照片分享社区：

- **登录（头像昵称）**：「个人」Tab 未登录展示登录卡，选择微信头像 + 输入昵称即登录；「个人」Tab 变为完整个人主页（资料、作品数/获赞、作品管理），可编辑资料；
- **照片流**：首页瀑布式照片流，可查看大图与详情，点赞、评论实时互动；
- **上传发布**：从相册选择/拍摄照片上传，云函数 `interactPhoto` 处理点赞评论事务，`getOpenid` 获取用户身份；
- **个人主页**：查看自己的作品与获赞统计，支持删除作品；
- **云函数封装**：数据读写、事务与权限均在云端完成，前端通过 `wx.cloud.callFunction` 调用。

## 技术要点

- **微信小程序（实验1-4、6）**：组件 `tabBar`、`swiper`、`canvas`、`picker`、`radio`、`button`、`scroll-view` 等；API `wx.navigateTo` / `wx.redirectTo` / `wx.switchTab` 页面跳转、`wx.getStorageSync` / `wx.setStorageSync` 本地缓存、`wx.createCanvasContext` 绘图、触摸事件、`wx.showActionSheet` / `wx.showModal` / `wx.showToast`、`wx.createInnerAudioContext` 音频、`onShareAppMessage` 分享、`chooseAvatar` 头像昵称填写、`wx.getFileSystemManager` 文件持久化；工程组织上做到数据与视图分离、全局登录状态共享（`globalData`）、缓存读写统一封装（`store.js`）；实验6 进一步使用**微信云开发**（`wx.cloud.init` / `callFunction`、云函数 Node.js、云数据库事务）；
...
**微信小程序（lab1 ~ lab4、lab6）**

1. 安装[微信开发者工具](https://developers.weixin.qq.com/miniprogram/dev/devtools/download.html)；
2. 使用开发者工具「导入项目」，选择本仓库中的 `lab1` ~ `lab4`、`lab6` 任意目录；
3. 实验3、实验4、实验6 需使用有效的小程序 AppID（`project.config.json` 中已配置）；实验6 还需开通**云开发**并在开发者工具中关联云环境（`cloudfunctions/` 目录右键上传部署云函数）；
...
- [实验6 报告](实验6.md) · [CSDN 博客](https://blog.csdn.net/natiedog/article/details/164747772)
